...
export async function POST(req: Request) {
    ...
    try {
        ...
        const report = {
            ...
            businessLogic: {
                ...
                reputation: {
                    ...
                    reviewCount: "0",
                    source: "Unknown",
                    reviews: [] as any[] // Add reviews field
                }
            },
            content: {
        ...
        try {
            ...
            // 4. GBP Risk Check & Review Analysis (Using Maps Scraper)
            // Use the scraper we built
            const { scrapeGoogleMaps } = require('@/lib/maps_scraper');
            const scraperTerm = `${businessName} ${cityGuess}`;
            debugLog(`Scraping Maps for: ${scraperTerm}`);
            ...
            try {
                const mapsData = await scrapeGoogleMaps(scraperTerm);
                if (mapsData) {
                    rating = mapsData.rating?.toString() || "0";
                    count = mapsData.reviewCount?.toString() || "0";
                    reviewsList = mapsData.reviews || [];
                    source = "Google Maps";
                    debugLog(`Maps Scraper Found: ${rating} stars, ${count} reviews.`);
                }
            } catch (scraperErr: any) {
                debugLog(`Maps Scraper Failed: ${scraperErr.message}`);
                // Fallback to old Google Search snippet method if Scraper fails completely?
                // For now, let's trust the scraper or leave as 0.
            }
            ...
            report.businessLogic.reputation = {
                ...
                reviewCount: count,
                source: source,
                // Pass reviews to frontend (need to update interface potentially, or just store in raw)
                reviews: reviewsList
            };
